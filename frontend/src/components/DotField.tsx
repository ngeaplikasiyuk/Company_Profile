...
const DotField = memo(({
  ...
  useEffect(() => {
    ...
    function buildDots(w: number, h: number) {
      ...
      const radius = p.dotRadius ?? 1.5;
      const spacing = p.dotSpacing ?? 14;
    ...
    function tick() {
      ...
      const crSq = cr * cr;
      const dotRadiusVal = p.dotRadius ?? 1.5;
      const rad = dotRadiusVal / 2;
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (distSq < crSq && eng > 0.01) {
          ...
          if (isBulge) {
            ...
            const push = t * t * bulgeStr * eng;
            const angle = Math.atan2(dy, dx);
            d.sx += (d.ax - Math.cos(angle) * push - d.sx) * 0.15;
            d.sy += (d.ay - Math.sin(angle) * push - d.sy) * 0.15;
          } else {
            const angle = Math.atan2(dy, dx);
            const force = p.cursorForce ?? 0.1;
            const move = (500 / dist) * (m.speed * force);
